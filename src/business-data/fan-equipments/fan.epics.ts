...
export const fetchMachinesEpic = (action$: ActionsObservable<Action>, store: Store<AppState>, deps: EpicDependencies) => {
  return action$.ofType(FanMachineActions.FETCH_FAN_MACHINE_EQUIPMENTS_DATA)
    .switchMap((action) => {
      console.log('FanMachineActions.FETCH_FAN_MACHINE_EQUIPMENTS_DATA');
      return deps.db.executeSql(`select * from ${tableNames.eam_sync_actions} where syncAction=?`, [action.type])
        ...
        .map(res => {
          ...
          return { action, lastSyncTime };
        })
        .switchMap(({ action, lastSyncTime }) =>
          deps.http.get(deps.mroApis.getCurServerTimeApi)
        ...
        .switchMap(({ actionLastSyncTimeCurServerTime, machinesRes }) => {
          ...
          return Observable.empty().startWith('fetchMachines')
            ...
            .mergeMap((machineDetails: [FanMachineEquipmentDetails[]]) => {
              ...
              const insertMachineSql = `insert into ${tableNames.eam_sync_fan_machine_equipment}( id, machineTypeName, machineId, machineTypeId, projectId, projectName, positionId, areaCode, areaName, positionCode, fanMachineJson )values(?,?,?,?,?,?,?,?,?,?,?)`;
              ...
              id, machineId, equipmentTreeJson, equipmentsDetailsJson, fanMachineInfo )values(?,?,?,?,?)`;
